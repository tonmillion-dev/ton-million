    ...
    it('random test', async () => {
        ...
        for (let i = 1; i <= round; i++) {
            ...
            for (let j = 1; j <= player; j++) {
                const randomFlag = Math.floor(Math.random() * 1000) % 2 + 1;
                console.log(`## Deposit user-${j}, flag=${randomFlag} ##`);
            ...
            // const winning = await roundContract.getWinnings();
            ...
            for (let j = 1; j <= player; j++) {
                console.log(`## Claim user-${j}, round=${i} ##`);
                depositor = await blockchain.treasury('ton_depositor' + j);
                if (await roundContract.getIsAnswer(depositor.address)) {
                    const winning = await roundContract.getWinnings();
                    console.log(`Round-${i}, user-${j} is answer! Earn: ${winning}, gas: ${toNano("0.05")}`);
                    answerCount[j]++;
                    const tryClaim = await roundContract.send(depositor.getSender(),
                        {
                            value: toNano("0.05")
                        },
                        {
